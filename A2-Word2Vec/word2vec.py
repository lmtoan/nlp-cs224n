...
def naiveSoftmaxLossAndGradient(
    ...
    gradOutsideVecs[neg_mask, :] = sm[neg_mask].reshape(-1, 1) * np.tile(centerWordVec, (len(sm[neg_mask]), 1))
...
def negSamplingLossAndGradient(
    ...
    # correctSM = sigmoid(outsideVectors[outsideWordIdx].dot(centerWordVec))
    # negSM = sigmoid(-outsideVectors[negSampleWordIndices].dot(centerWordVec))
    # loss = -np.log(correctSM) - np.sum(np.log(negSM), axis=0)
    # gradCenterVec = (correctSM - 1) * outsideVectors[outsideWordIdx] - outsideVectors[negSampleWordIndices].T.dot(negSM - 1).flatten()
    # gradOutsideVecs[outsideWordIdx, :] = (correctSM - 1) * centerWordVec
    # gradOutsideVecs[negSampleWordIndices, :] = (1 - negSM).reshape(-1, 1) * np.tile(centerWordVec, (len(negSampleWordIndices), 1))
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for word in outsideWords:
        lossCurrent, grad_center, grad_outside = word2vecLossAndGradient(centerWordVec=centerWordVectors[word2Ind[currentCenterWord]],
...
def test_word2vec():
    ...
    gradcheck_naive(lambda vec: word2vec_sgd_wrapper(
        ...
        dummy_vectors, "naiveSoftmaxLossLossAndGradient Gradient")
    grad_tests_softmax(skipgram, dummy_tokens, dummy_vectors, dataset)

    print("==== Gradient check for skip-gram with negSamplingLossAndGradient ====")
    gradcheck_naive(lambda vec: word2vec_sgd_wrapper(
        skipgram, dummy_tokens, vec, dataset, 5, negSamplingLossAndGradient),
        dummy_vectors, "negSamplingLossLossAndGradient Gradient")
