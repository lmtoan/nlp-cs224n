...
def pad_sents_char(sents, char_pad_token):
    ...
    sents_padded = []
    pad_word = [char_pad_token for _ in range(max_word_length)]
    max_sentence_length = max([len(s) for s in sents])
    for sid in sents:
        sid_padded = []
        for wid in sid:
            wid = wid[:min(len(wid), max_word_length)]
            wid += (max_word_length - len(wid)) * [char_pad_token]
            assert len(wid) == max_word_length
            sid_padded.append(wid)
        for _ in range(max_sentence_length - len(sid_padded)):
            sid_padded.append(pad_word)
        assert len(sid_padded) == max_sentence_length
        sents_padded.append(sid_padded)
    # END YOUR CODE
    return sents_padded
