...
class NMT(nn.Module):
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        source_padded_chars = self.vocab.src.to_input_tensor_char(source, device=self.device)   # Tensor: (src_len, b, maxword_len)
        target_padded_chars = self.vocab.tgt.to_input_tensor_char(target, device=self.device)   # Tensor: (tgt_len, b, maxword_len)
        # emb(source_padded_chars) = (src_len, b, emb) --> enc_hidden (b, src_len, hidden), dec_init_state = pair of (b, hidden)
        enc_hiddens, dec_init_state = self.encode(source_padded_chars, source_lengths)
        enc_masks = self.generate_sent_masks(enc_hiddens, source_lengths)  # enc_masks = (b, hidden) but every sentence, words beyond its original length masked as 1 (-inf) else 0
        combined_outputs = self.decode(enc_hiddens, enc_masks, dec_init_state, target_padded_chars)  # (tgt_len, b, hidden)
        ...
        if self.charDecoder is not None:
            ...
            target_words = target_padded[1:].contiguous().view(-1)  # Skip <start> token, flatten to (tgt_len*b)
            target_chars = target_padded_chars[1:].contiguous().view(-1, max_word_len)  # Skip <start>, flatten to (tgt_len*b, maxword_len)
            target_outputs = combined_outputs.view(-1, 256)  # (tgt_len*b, hidden)
    ...
    def beam_search(self, src_sent: List[str], beam_size: int = 5, max_decoding_time_step: int = 70) -> List[Hypothesis]:
        ...
        while len(completed_hypotheses) < beam_size and t < max_decoding_time_step:
            ...
            if len(decoderStatesForUNKsHere) > 0 and self.charDecoder is not None:  # decode UNKs
                ...
                decodedWords = self.charDecoder.decode_greedy((decoderStatesForUNKsHere.unsqueeze(0), decoderStatesForUNKsHere.unsqueeze(0)),
