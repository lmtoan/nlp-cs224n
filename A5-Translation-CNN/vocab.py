...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        for sentence in sents:
            ...
            for word in sentence:
                wids = [self.char2id[ch] for ch in word]
                wids = [self.start_of_word] + wids + [self.end_of_word]
                sentence_ids.append(wids)
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        # previous parts
        charsets = self.words2charindices(sents)
        padded_charsets = pad_sents_char(charsets, self.char2id['<pad>'])
        padded_charsets = torch.LongTensor(padded_charsets).to(device)  # batch_size, sent_length, word_length
        padded_charsets = torch.transpose(padded_charsets, 0, 1).contiguous()
        return padded_charsets
        # END YOUR CODE
